...
class _NotificationScreenState extends State<NotificationScreen> {
  ...
  void initState() {
    ...
    final isSubscriptionExpired = authState is AuthAuthenticated &&
        SubscriptionHelper.isSubscriptionExpired(authState.user);
  ...
  Widget build(BuildContext context) {
    ...
    final isSubscriptionExpired =
        user != null && SubscriptionHelper.isSubscriptionExpired(user);
        ...
        child: SafeArea(
          child: Padding(
            ...
            child: isSubscriptionExpired
                ...
                : _SmsContent(patId: widget.patId),
          ),
        ),
      ),
    );
...
class _SubscriptionExpiredContent extends StatelessWidget {
  ...
  const _SubscriptionExpiredContent({required this.onRenew});
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Votre abonnement a expiré. Veuillez renouveler votre abonnement pour consulter les SMS reçus.',
...
class _SmsContent extends StatelessWidget {
  final String patId;

  const _SmsContent({required this.patId});
            ...
            builder: (context, state) {
              ...
              if (state is SmsLoaded) {
                ...
                return RefreshIndicator(
                  onRefresh: () async {
                    context.read<SmsBloc>().add(LoadSmsRecus(patId: patId));
                  },
