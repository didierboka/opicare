...
class DestinationRemoteDataSourceImpl implements DestinationRemoteDataSource {
  ...
  Future<List<DestinationModel>> getDestinations() async {
    try {
      ...
      MyLogger.writeLog("RÉPONSE DES DESTINATIONS -> $response");
  ...
  List<DestinationItemModel> _parseDestinations(String destinationsString) {
    try {
      ...
      return items.where((item) => item.trim().isNotEmpty).map((item) => DestinationItemModel.fromString(item)).toList();
    } catch (e) {
  ...
  @override
  Future<String?> getDestinationDetails(String id) async {
    try {
      ...
      final response = await _apiService.post(
        ...
        {
          'optionID': id,
          'transactionID': "1756070969425",
        },
        likeOrange: true,
        useFormData: false
      );

      DebugLogger.network('Réponse des détails de destination -> ID $id');

      if (response.response == null) {
        throw Exception('Réponse vide du serveur pour les détails de la destination');
      }

      DebugLogger.log('Réponse non vide');

      final responseModel = DestinationDetailsResponse.fromJson(response.response!);

      DebugLogger.log('Réponse non vide formattee');
      DebugLogger.debug(jsonEncode(responseModel));
      ...
      return responseModel.messages.first;
    } catch (e) {
      DebugLogger.log('Erreur dans getDestinationDetails: $e');
